        ...
        "test_cases": [
            {
                ...
                "type": r.type,
                # category/feature_area: use getattr with a fallback rather than
                # r.category/r.featureArea directly — if this deploys before
                # `prisma generate` has been re-run against the migrated schema,
                # direct attribute access on the Prisma-generated model would
                # throw AttributeError and take down the whole endpoint instead
                # of just missing these two fields.
                "category": getattr(r, "category", None) or "functional",
                "feature_area": getattr(r, "featureArea", None),
                "created_at": r.createdAt,
