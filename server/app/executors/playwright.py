...
# Variant count per intent — "how many test cases does this workflow
# actually deserve." Deliberately NOT permutation-based (e.g. login doesn't
# get one variant per possible username/password pair — that's just token
# spend with no new coverage). Two tiers:
#
#   CAPPED LOW (auth/simple, regardless of how "important" it feels):
#   login/signup max out at 3 — one valid pass, plus 1-2 invalid-credential
#   variants. Testing 6 different username/password combos covers the exact
#   same code path 6 times; it's waste, not coverage.
#
#   WEIGHTED HIGHER (genuinely multi-step / multi-field business flows):
#   checkout, add-to-cart, forms with several fields — these have real
#   branching (missing field X vs field Y, boundary quantities, cancel
#   mid-flow) so they're worth spending more generated test cases on.
_INTENT_VARIANTS = {
    # Auth — capped, not permutation-tested
    "login": 3, "signup": 3, "logout": 1,

    # High-value multi-step business flows
    "checkout": 6, "add_to_cart": 4, "form_submit": 4, "upload": 3,

    # Meaningful but single-purpose interactions
    "remove_from_cart": 3, "quantity_stepper": 3, "search": 3,
    "filter_sort": 3, "edit_update": 3, "delete": 3,

    # Low-branching / mostly-cosmetic interactions
    "toggle": 2, "pagination": 2, "download_export": 2, "generic_click": 2,

    # Pure navigation — one happy-path check is enough
    "nav_link": 1,
}
...
async def _extract_page_elements(page, page_url: str) -> list:
    ...
    Each element gets a unique data-scout-id attribute stamped onto the live
    DOM node — this gives exploratory clicking (see _explore_clickable_elements)
    a reliable way to re-locate the exact element later via CSS selector,
    instead of a fragile text-match that breaks on icons/duplicate labels."""
    try:
...
def _process_page_elements(elements: list, route_pattern: str, url: str, clusters: dict,
    ...
    for el in elements:
        ...
        intent = _resolve_intent(tag, label)
        if intent is None:
...
def _resolve_intent(tag: str, label: str) -> str:
    """Same classify-then-fallback logic used everywhere an element's intent
    needs resolving, factored out so clustering and click-exploration never
    diverge on what counts as what."""
...
# Pure <a href> crawling misses most real navigation on modern apps — cart
# icons, checkout/continue buttons, and hamburger menus are almost always
# JS-driven (href="#" or a bare <button>, no real href to follow). Without
# clicking through them, the scout only ever sees whatever page it happened
# to land on directly, which is why SwagLabs showed just 2 pages / 6
# workflows despite a 15-page crawl limit.
#
# This is scoped deliberately conservatively: only elements whose intent is
# NOT in the excluded set, and whose label doesn't match a destructive-
# sounding keyword, are ever clicked. Anything that mutates data in place
# (delete, remove from cart, logout, upload) or looks like a final/
# irreversible action (Place Order, Pay, Confirm, Finish, Submit) is counted
# as a workflow from static extraction alone — it is NEVER auto-clicked.
_CLICK_EXCLUDED_INTENTS = {
    "delete", "remove_from_cart", "logout", "upload", "download_export",
    "edit_update", "add_to_cart", "quantity_stepper",
}

_DESTRUCTIVE_LABEL_KEYWORDS = (
    "place order", "pay now", "pay ", "confirm order", "confirm payment", "confirm",
    "submit order", "submit payment", "submit", "finish", "complete order",
    "complete purchase", "purchase", "delete", "remove", "unsubscribe", "cancel subscription",
)


def _is_destructive_label(label: str) -> bool:
    text = (label or "").strip().lower()
    return any(k in text for k in _DESTRUCTIVE_LABEL_KEYWORDS)


async def _explore_clickable_elements(page, elements: list, current_route: str, current_url: str,
                                       clusters: dict, origin_netloc: str, queue: list,
                                       queued_urls: set, visited_routes: set, page_limit: int,
                                       stats: dict) -> None:
    """Best-effort click-through discovery for JS-driven navigation (buttons
    and href="#"/javascript: links) that pure link-following can't see —
    cart icons, checkout/continue buttons, hamburger menus. Strictly scoped
    to the safe-list above. Capped at 6 clicks per page to bound crawl time."""
    if stats["pages_scanned"] >= page_limit:
        return
    ...
    for el in elements:
        ...
        href = el.get("href")
        label = el.get("label") or ""
        scout_id = el.get("scoutId")
        if not scout_id:
            continue
        is_real_link = tag == "a" and href and href not in ("#", "javascript:void(0)", "javascript:;", "")
        if is_real_link:
            continue  # already handled by normal href crawling — no need to click it too
        intent = _resolve_intent(tag, label)
        if intent is None or intent in _CLICK_EXCLUDED_INTENTS:
            continue
        if _is_destructive_label(label):
            continue
        candidates.append((scout_id, label))

    for scout_id, label in candidates[:6]:
        if stats["pages_scanned"] >= page_limit:
            break
        try:
            locator = page.locator(f'[data-scout-id="{scout_id}"]')
            ...
            if after_url != before_url:
                # Real navigation happened (JS-driven, so <a href> crawling
                # would never have found this) — treat it exactly like a
                # discovered link.
                new_route = _normalize_route(urlparse(after_url).path)
                if new_route not in visited_routes and stats["pages_scanned"] < page_limit:
                    new_elements = await _extract_page_elements(page, after_url)
                    stats["total_elements"] += len(new_elements)
                    _process_page_elements(new_elements, new_route, after_url, clusters,
                                            origin_netloc, queue, queued_urls, visited_routes)
                    visited_routes.add(new_route)
                    stats["pages_scanned"] += 1
                # Restore the original page before trying the next candidate,
                # so exploration doesn't compound state across clicks.
                try:
                ...
                # No navigation — likely revealed hidden content in place
                # (e.g. a hamburger menu opening). Re-extract and merge in
                # anything that's now visible that wasn't before.
                revealed = await _extract_page_elements(page, current_url)
                stats["total_elements"] += len(revealed)
                _process_page_elements(revealed, current_route, current_url, clusters,
                                        origin_netloc, queue, queued_urls, visited_routes)
                try:
                    await page.keyboard.press("Escape")  # best-effort close for menus/overlays
                except Exception:
                    pass
        except Exception as e:
            print(f"[Scout] exploratory click on '{label}' failed: {e}")
            continue
...
async def _scout_application(base_url: str, page_limit: int, login_fields: dict = None) -> dict:
    ...
    queue: list = []
    queued_urls: set = set()
    # Shared mutable counters — passed into _explore_clickable_elements too,
    # since exploratory clicks can discover (and need to count) whole new
    # pages of their own, not just elements on the current page.
    stats = {"pages_scanned": 0, "total_elements": 0}
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if login_fields:
                try:
                    ...
                    if await page.locator('input[type="password"]').count() > 0:
                        ...
                        if auth_succeeded and stats["pages_scanned"] < page_limit:
                            ...
                            if post_login_route not in visited_routes:
                                ...
                                stats["total_elements"] += len(elements2)
                                _process_page_elements(elements2, post_login_route, post_login_url, clusters,
                                                        origin_netloc, queue, queued_urls, visited_routes)
                                visited_routes.add(post_login_route)
                                stats["pages_scanned"] += 1
                                # This is where the payoff is: the authenticated
                                # landing page is exactly where cart icons,
                                # hamburger menus, and checkout-entry buttons
                                # live — all JS-driven, all invisible to plain
                                # href crawling.
                                await _explore_clickable_elements(
                                    page, elements2, post_login_route, post_login_url, clusters,
                                    origin_netloc, queue, queued_urls, visited_routes, page_limit, stats
                                )
                except Exception as e:
            ...
            # ── Main BFS crawl ────────────────────────────────────────────
            while queue and stats["pages_scanned"] < page_limit:
                url = queue.pop(0)
                parsed = urlparse(url)
                ...
                _process_page_elements(elements, route_pattern, url, clusters,
                                        origin_netloc, queue, queued_urls, visited_routes)
                # Click through JS-driven navigation on this page too (cart
                # icons, checkout buttons, menus) — not just pages reached by
                # <a href>, following the same strict safe-list every time.
                await _explore_clickable_elements(
                    page, elements, route_pattern, url, clusters,
                    origin_netloc, queue, queued_urls, visited_routes, page_limit, stats
                )
        finally:
            await context.close()
