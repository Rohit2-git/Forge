...
export const apiService = {
  ...
  generateTestPack: async (
    ...
    // No more user-chosen count — the AI decides how many test cases this
    // input needs (server-side, bounded by MIN/MAX_BLUEPRINTS). existingTitles
    // is how the backend avoids repeating scenarios from earlier batches
    // generated for this app in the current session.
    if (existingTitles && existingTitles.length > 0) {
      formData.append('existing_titles', JSON.stringify(existingTitles));
    }
