...
export const TestDataManager: React.FC = () => {
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      ...
      // This used to fail with zero visible feedback — the click looked
      // like it did nothing. A common cause: the active application isn't
      // actually saved server-side yet (e.g. a prior save silently failed),
      // so this template has nowhere valid to attach to.
      console.error('Failed to save template:', err);
      setTemplateError(err?.message || 'Could not save this template — check that the active application was saved successfully, then try again.');
    } finally {
  ...
  const handleSaveCondition = async () => {
    ...
    try {
      ...
      setConditionError(err?.message || 'Could not save this condition — check that the active application was saved successfully, then try again.');
    } finally {
          ...
          <div style={{
            ...
            {showTemplateForm && (
              <div style={{ padding: '1.5rem' }}>
                ...
                {templateError && (
                  <p style={{ fontSize: '0.8rem', color: 'var(--color-error)', marginBottom: '1rem' }}>{templateError}</p>
                )}
          ...
          <div style={{
            ...
            {showConditionForm && (
              <div style={{ padding: '1.5rem' }}>
                ...
                {conditionError && (
                  <p style={{ fontSize: '0.8rem', color: 'var(--color-error)', marginBottom: '1rem' }}>{conditionError}</p>
                )}
